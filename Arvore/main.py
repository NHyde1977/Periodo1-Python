...
from ArvoreManual import resultado
...
# Your code here!!
# def getMessage(codmorse, root):
#     msg = ""
#     node = root
#     for simbolo in codmorse:
#         if simbolo == ".":
#             node = node.left
#         elif simbolo == "-":
#             node = node.right
...
def decodeletra(morseCode, raiz):

    if not morseCode:
        return raiz.value
    elif morseCode[0] == ".":
        return decodeletra(morseCode[1:], raiz.left)
    elif morseCode[0] == "-":
        return decodeletra(morseCode[1:], raiz.right)

def decodefullmorse(morseCode, raiz):
    letrasmorse = morseCode.strip().split(" ")
    palavra = ""

    for pontotraco in letrasmorse:
        letra = decodeletra(pontotraco, raiz)
...
print(morseCode)
#msgdecodificada = getMessage(morseCode, tree)
# print(msgdecodificada)
print(decodefullmorse(morseCode, tree))
